...
import { analyzeTaste, analyzeProductTaste, TasteWarning } from '@/lib/tasteWarnings';
...
export default function Panier() {
 ...
 const { cart, removeFromCart, updateQuantity, clearCart, getTotalPrice, updateCartItemIngredients, updateCartItemPrice } = useCart();
  
  // États pour le pop-up d'avertissement gustatif
...
// Re-analyser automatiquement quand le panier change
useEffect(() => {
  if (showWarningDialog && cart.length > 0) {
    ...
    // Analyser les ingrédients du produit (jus du menu)
if (item.ingredients && item.ingredients.length > 0) {
  const warnings = analyzeTaste(item.ingredients);
  allWarnings = [...allWarnings, ...warnings];
}
...
const handleCheckout = () => {
  ...
  cart.forEach(item => {
    // Analyser TOUS les items qui ont des ingrédients
    if (item.ingredients && item.ingredients.length > 0) {
      const warnings = analyzeTaste(item.ingredients);
      allWarnings = [...allWarnings, ...warnings];
    }
  ...
  // Dédupliquer les avertissements par type
  const uniqueWarnings = allWarnings.reduce((acc: TasteWarning[], current) => {
    ...
    if (!exists) {
      ...
      // Fusionner les ingrédients si même type d'avertissement
      exists.ingredients = [...new Set([...exists.ingredients, ...current.ingredients])];
      return acc;
    }
  }, []);
...
const handleRemoveIngredient = (ingredientName: string) => {
  ...
  cart.forEach(item => {
    // CAS 1 : Produit du menu sans ingrédients modifiables
    ...
    // CAS 2 : Produit avec ingrédients
    if (item.ingredients && item.ingredients.length > 0) {
      const hasIngredient = item.ingredients.some(ing => ing.name === ingredientName);
      
      if (hasIngredient) {
        // Si c'est le dernier ingrédient, supprimer tout le produit
        if (item.ingredients.length === 1) {
          ...
          // Recalculer le prix (prix de base + prix des add-ons restants)
          const removedIngredient = item.ingredients.find(ing => ing.name === ingredientName);
          const newPrice = item.basePrice - (removedIngredient?.price || 0);
          
          // Mettre à jour
          updateCartItemIngredients(item.id, updatedIngredients);
          updateCartItemPrice(item.id, newPrice); // Nouvelle fonction à créer
          somethingChanged = true;
        }
      }
    }
...
<div className="flex items-start justify-between">
  <div className="flex-1">
    ...
    <div className="mt-3 pt-3 border-t border-gray-200">
      <div className="flex justify-between items-center text-sm">
        ...
        <span className="font-semibold text-[#004D40]">{item.basePrice.toFixed(2)}€</span>
      </div>
      {item.quantity > 1 && (
        <div className="flex justify-between items-center text-sm mt-1">
          <span className="text-gray-600">Total ({item.quantity}x):</span>
          <span className="font-bold text-[#FF6F00]">{(item.basePrice * item.quantity).toFixed(2)}€</span>
        </div>
      )}
    </div>
  </div>
  ...
  <div className="flex flex-col items-end gap-4">
    ...
    >
      ...
      {/* Dialog d'avertissement gustatif */}
      <TasteWarningDialog
